...
# 2. IMPORT DATASETS
# -------------------------
full_data <- read.csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vR8Br2GEyE2-YtJS7Dd2Y0IfmsPH3lSvQhAabFlDBq0w2kl_elw6gMNj79q-IGMx3uL8gfzwx_GlSkm/pub?gid=1906655143&single=true&output=csv")

curated_data <- read.csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vR8Br2GEyE2-YtJS7Dd2Y0IfmsPH3lSvQhAabFlDBq0w2kl_elw6gMNj79q-IGMx3uL8gfzwx_GlSkm/pub?gid=1939872776&single=true&output=csv")
...
model_takeoff_temp_simple <- lmer(
  Take_off_Temperature ~ Bee_species + Take_off_T_ambient + (1|Date),
  data = full_data
)
summary(model_takeoff_temp_simple)
...
# Righting duration (final reduced model)
...
model_takeoff_duration <- lmer(
  log_takeoff ~ ITD_Final + Take_off_T_ambient + (1|Date),
  data = full_data
)
summary(model_takeoff_duration)
